...
- **2026-07-12** — Sesión legal/fiscal profunda. Asesoría sobre: SAS vs otras sociedades, no afecta trabajo independiente de datos, capital suscrito bajo, préstamos (mutuo) vs capital, cómo consignar con trazabilidad, Régimen Simple (RST), inversionista extranjero (Banco de la República), cuentas bancarias (Bancolombia/BBVA IMC + Wompi, no Bold), envíos internacionales (EMS) y nacionales (Servientrega/Interrapidísimo). Creados en `docs/` (privado): `guia-legal-fiscal.md`, `contrato-mutuo-plantilla.md`, y **`Guia-Legal-Seoul-Drop.docx`** (documento Word maestro para principiantes, 22 secciones + glosario + checklist). Todo pensado legal e inteligente (sin evasión).
